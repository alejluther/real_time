...
wait_queue_head_t virt_dev_queue_wait;
...
#if 0
	/* process will be put to sleep in the queue if no data */
	wait_event_interruptible(virt_dev_queue_wait, total_count != 0);
#endif
...
{
	int i, ret;
	printk("INIT_MODULE: Exercise 05\n");

	init_waitqueue_head(&virt_dev_queue_wait);
...
{
    printk("CLEANUP_MODULE: Ejercicio driver 05\n");
...
MODULE_DESCRIPTION("LINUX DRIVERS TRAINING: Exercise 05");
MODULE_LICENSE("GPL");
